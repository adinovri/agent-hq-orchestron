...
const BASE_URL = (values.url as string) ?? 'http://localhost:8080'
...
function App() {
  ...
  const onNew = useCallback(() => {
    setStatusMsg('Spawn not implemented in TUI — use CLI: orchestron session spawn')
  ...
  return (
    <Box flexDirection="column" padding={0}>
      ...
      {screen === 'detail' && selectedSession && (
        <SessionDetail session={selectedSession} config={config} onBack={onBack} />
      )}
