...
/** Stand-in transcript. Reopen and Fork both refuse a session with no
 *  transcript on disk, which is a different gate from the mode flag this
 *  suite is about — so the file has to exist for those paths to be
 *  reachable at all. */
const MOCK_JSONL = path.join(os.tmpdir(), 'orchestron-headless-flag-mock.jsonl')
...
function makeAdapter(): AgentAdapter {
  spawnSpy = vi.fn().mockResolvedValue({
    ...
    jsonlPath: MOCK_JSONL,
  })
  return {
    name: 'mock',
    spawn: spawnSpy,
    // Reopen and Fork now actually reach the adapter — they used to 409 in
    // this suite before the mode flag ever mattered — so resume has to
    // return a usable handle rather than undefined.
    resume: vi.fn(async (uuid: string) => ({
      tmuxName: 'mock-resumed',
      claudeUuid: uuid,
      jsonlPath: MOCK_JSONL,
    })),
...
beforeEach(() => {
  ...
  fs.writeFileSync(MOCK_JSONL, '{"type":"user"}\n')
})
...
describe('reopen and fork ARE coerce sites', () => {
  // They were exempt while reopening a headless session was refused
  // outright — a coercion would have slipped past that gate without making
  // the resume safer. Cross-mode resume is now verified on both harnesses
  // and the refusal is gone, so there is no gate left to slip past and both
  // are ordinary places a session could end up headless.
  it('reopens a headless session into tmux while the flag is off', async () => {
    const { h, id } = await terminalRecordThen(false, false)
    harness = h
    const res = await h.app.inject({
  ...
  it('forks a headless session into tmux while the flag is off', async () => {
    const { h, id } = await terminalRecordThen(false, false)
    harness = h
    const res = await h.app.inject({
  ...
  it('does not flag a coercion when the session was tmux anyway', async () => {
    // `coerced` is the signal to raise a toast, so it must appear only when
    // the user actually got something other than what the record asked for.
    const { h, id } = await terminalRecordThen(true, false)
    harness = h
    const res = await h.app.inject({
      method: 'POST', url: `/api/sessions/${id}/reopen`, payload: {},
